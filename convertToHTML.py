...
def convertKeyParam(resultsFile,LZ):
    ...
    if str(jsonData["awsNumberIamUsers"])!='n/a' and int(jsonData["awsNumberIamUsers"])>0:
        ...
        for key in jsn_list:
            html=html+"<LI><B>      " + key + "</B> : " + str(jsn_list[key]) + " </LI>\n"
        html=html+"</UL>"
   
    html=html+ "<hr class=\"dashed\">\n"
    html=html+ "<H2>Accounts</H2>\n"
    html=html+"<P><B>Number of accounts in LZ" + LZ + "</B> : " +str(jsonData["awsTotalNumberAccounts"]) + " </P>\n"
    if str(jsonData["awsTotalNumberAccounts"])!='n/a':
        html=html+ "<H3>List of Accounts- <I> Name : Arn</I></H3>\n"
        html=html+"<UL>"
        jsn_list = jsonData['List_of_Accounts']
        for key in jsn_list:
    ...
    if str(jsonData["numberLambdaFunctions"])!='n/a':
    ...
    if str(jsonData["awsNumberRoles"])!='n/a':
    ...
    if str(jsonData["numberOfTaskDefinition"])!='n/a':
