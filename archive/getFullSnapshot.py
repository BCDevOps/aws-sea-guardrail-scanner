...
for n in range(int(numberOfAccounts)):
    ...
    # Originally the roles node was like
    #   "709720252926": {
    #               "licensePlate":"ubwyx8-sandbox",
    #               "roles":[
    #                   "BCGOV_WORKLOAD_admin_tmhl5tvs",
    #                   "BCGOV_WORKLOAD_billing_viewer_tmhl5tvs",
    #               ]
    #           }
    #     
    # so it would scan each role. However, it is wrong, as it always use the only aws role with a trust relationship with the 
    # master account, that is AWSCloudFormationStackSetExecutionRole
    #
    ...
    for j in range(numberRoles):
        ...
        os.system('aws sts assume-role --role-arn arn:aws:iam::' + userAccount +':role/AWSCloudFormationStackSetExecutionRole --role-session-name test | jq \'.Credentials | {AccessKeyId,SecretAccessKey,SessionToken}\' > ./apiResults.json')
